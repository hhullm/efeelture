...
public class HardwareDaoImpl implements HardwareDao{
	...
	public void addHardware(Hardware hardware) {
		...
		try {
			...
			PreparedStatement stat = conn
					.prepareStatement("insert into db_hardware_match(id,uid,hardwareid,htime) VALUES(?,?,?,?)");
	...
	public void deleteHardware(Hardware hardware) {
		...
		try {
			conn = DBUtil.getConnection();
			PreparedStatement stat = conn.prepareStatement("delete from db_hardware_match where id=?");
			stat.setString(1, hardware.getId());
			stat.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void modifyHardware(Hardware hardware) {
		...
		try {
			...
			conn = DBUtil.getConnection();
			PreparedStatement stat = conn
					.prepareStatement("update db_hardware_match set uid=?,hardwareid=?,htime=? where id=?");
	...
	private String getSql(Hardware hardware) {
		// TODO Auto-generated method stub
		String sql = "select * from db_hardware_match";
		if (hardware != null) {
			sql += " WHERE 1=1 ";
			if (hardware.getId() != null && !hardware.getId().equals(""))
